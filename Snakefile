...
rule minimap2:
    ...
    conda:
        "envs/minimap2.yml"
    shell:
        "minimap2 -ax map-ont reference_sequences/all_referencesCO1.txt {input} > {output}"
...
&& samtools view -b -F 4 output_data/samtools_data/aln_sorted_{wildcards.sample}.bam > output_data/samtools_data/extracted_reads_{wildcards.sample}.bam && \
...
bcftools mpileup -f reference_sequences/all_referencesCO1.fasta output_data/samtools_data/extracted_reads_{wildcards.sample}.bam -o output_data/samtools_data/mpileup_{wildcards.sample}.vcf && \
bcftools call -c output_data/samtools_data/mpileup_{wildcards.sample}.vcf -o output_data/samtools_data/called_variants_{wildcards.sample}.vcf && \
vcfutils.pl vcf2fq output_data/samtools_data/called_variants_{wildcards.sample}.vcf > {output}"
